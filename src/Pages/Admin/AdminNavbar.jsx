                ...
                {menuItems.map((item, index) => {
                  ...
                  return (
                    <Link
                      key={index}
                      to={item.href}
                      className={`group relative flex items-center px-5 py-3 rounded-2xl text-sm font-medium transition-all duration-300 hover:scale-105 transform ${
                        item.name === 'Logout' 
                          ? 'hover:bg-red-900/20 hover:border-red-500/30 hover:shadow-lg hover:shadow-red-500/10' 
                    ...
                    >
                      ...
                      <IconComponent className={`w-5 h-5 mr-3 ${item.color} group-hover:animate-pulse relative z-10 group-hover:drop-shadow-lg`} />
                      <span className="text-gray-300 group-hover:text-white relative z-10 font-medium">
                        {item.name}
                      </span>
                      {/* Bottom border animation */}
                      <div className={`absolute bottom-0 left-1/2 w-0 h-0.5 ${
                        item.name === 'Logout' ? 'bg-red-400' : 'bg-gradient-to-r from-cyan-400 to-purple-400'
                      } group-hover:w-full group-hover:left-0 transition-all duration-300 rounded-full`}></div>
                    </Link>
                  );
                })}
              </div>
              ...
              {menuItems.map((item, index) => {
                ...
                return (
                  <Link
                    key={index}
                    to={item.href}
                    className={`group relative flex items-center px-5 py-4 rounded-2xl text-base font-medium transition-all duration-300 ${
                      item.name === 'Logout' 
                        ? 'hover:bg-red-900/20 hover:border-red-500/30 hover:shadow-lg hover:shadow-red-500/10' 
                        : 'hover:bg-white/5 hover:shadow-lg hover:shadow-purple-500/10'
                    } border border-transparent hover:border-white/10 backdrop-blur-sm`}
                  ...
                  >
                    ...
                    <IconComponent className={`w-6 h-6 mr-4 ${item.color} group-hover:animate-pulse relative z-10 group-hover:drop-shadow-lg`} />
                    <span className="text-gray-300 group-hover:text-white relative z-10 font-medium">
                      {item.name}
                    </span>
                    {/* Side border animation */}
                    <div className={`absolute left-0 top-1/2 h-0 w-1 ${
                      item.name === 'Logout' ? 'bg-red-400' : 'bg-gradient-to-b from-cyan-400 to-purple-400'
                    } group-hover:h-full group-hover:top-0 transition-all duration-300 rounded-r-full`}></div>
                  </Link>
                );
              })}
            </div>
